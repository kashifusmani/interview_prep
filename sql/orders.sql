...
---TELEHEALTH
--select customer who did not place an order and products that did not sell


select id , customer_name as name, "customer" as category
from customer
where id not in (select distinct(customer_id) from invoice)
union
select id, product_name as name, "product" as category
from product where id not in (select distinct(product_id) from invoice_item)
...
WHERE
    O.ORDER_DATE <= DATE_ADD((SELECT MIN(ORDER_DATE) MIN_ORDER_DATE FROM ORDERS), interval 10 year)
